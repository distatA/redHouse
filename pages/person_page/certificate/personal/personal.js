...
Page({
  ...
  onShow: function () {
    if(this.data.type==0){
      this.getDetail()//详情 0-待城市审核 1-待平台审核 2-城市审核不通过 \3-平台审核通过 4-平台审核不通过
    }
    var that = this;
    wx.getStorage({
      key: 'idNum',
      ...
      common.ajaxGet(
        ...
        function (data) {
          if (data.code == 200) {
            ...
            var regions = []
            regions.push( data.data.city, data.data.province)
...
// 提交
  sendAll: common.throttle(function(e){
    ...
    if (that.data.type == 0){//修改
      ...
      var nameValue = that.data.details.name;
      var codeValue = that.data.details.id_no;
      var phone = !that.data.phone ? that.data.details.mobile : that.data.phone;
      var code = that.data.code;
      var img1 = that.data.details.front_card;
      var img2 = that.data.details.reverse_card;
      var img3 = that.data.img3 == '' ? that.data.details.card_image : that.data.img3;
      var company_name = !that.data.company_name ? that.data.details.company_name : that.data.company_name;
    ...
    console.log(nameValue, codeValue, phone, img1, img2,code,img3)
    
    if (nameValue !=undefined){
      if (codeValue != undefined){
        if (common.validateIdCard(codeValue)){
          if (img1!=''){
            if(img2!=''){
              if (img3 !=''){
                ...
                wx.getStorage({
                  ...
                  success: function (res) {
                    wx.request({
                      url: app.globalData.URL + '/api/myRedAuth/peopleAuth/',
                      data: {
                        front_card: img1,//身份证正面照片地址
                        reverse_card: img2,//身份证反面照片地址
                        id_no: codeValue,//身份证号码
                        member_no: res.data.member_no,
                        mobile: phone,
                        name: nameValue,
...
//点击选择
  clickOn:function(e){
    ...
    // var changed = {}
    // for (var i = 0; i < this.data.arrList.length; i++) {
    //   if (checked.indexOf(this.data.arrList[i].num) !== -1) {
    //     changed['arrList[' + i + '].checked'] = true
    //   } else {
    //     changed['arrList[' + i + '].checked'] = false
    //   }
    // }
